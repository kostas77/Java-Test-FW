Feature: Product purchase E2E tests for UK regional health store via mobile
  As a customer
  I want to be able to purchase different product types (printbook, ebook, journal,...)
  So that they will be delivered to me

  @UKHealthStore @UKHS_E2E_Mobile @Mobile
  Scenario: UK-HS Print Book purchase using the existing account checkout path
    Given I navigate to the "UK" regional health store home page via mobile
    And I sign-in with username "test.elsevier.regional.ecom+uk@gmail.com" and password "Spoon!23" via mobile
  ...
  @UKHealthStore @UKHS_E2E_Mobile @Mobile
  Scenario: UK-HS E-Book purchase using the existing account checkout path
    Given I navigate to the "UK" regional health store home page via mobile
