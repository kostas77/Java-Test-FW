...
@Slf4j
public class productPurchaseEUSteps extends BasePage {
    ...
    public void i_add_the_e_book_to_the_cart() {
        ...
        Assertions.assertTrue(productPageActions.txtSuccessMessage.GetTextValue().contains("You added") || productPageActions.txtSuccessMessage.GetTextValue().contains("The maximum quantity you may purchase is limited to 5 print books and 1 single copy per eBook, with a limit of 5 eBooks per order."), "E-book added to cart message does not contain - You added");
        productPageActions.navigateToViewCartPage();
        DriverContext.waitForPageToLoad();
    ...
    public void i_search_for_different_product_and_under_product_type(DataTable table) {
        ...
        for (Map<String, String> productList : product) {
            ...
            if (ProductType.equals("E-Book") || ProductType.equals("E-Books")) {
                ...
                DriverContext.waitForPageToLoad();
                DriverContext.waitUntilElementIsVisible(productPageActions.txtSuccessMessage);
                Assertions.assertTrue(productPageActions.txtSuccessMessage.GetTextValue().contains("You added") || productPageActions.txtSuccessMessage.GetTextValue().contains("The maximum quantity you may purchase is limited to 5 print books and 1 single copy per eBook, with a limit of 5 eBooks per order.") || productPageActions.txtSuccessMessage.GetTextValue().contains("The maximum quantity you may purchase is limited to 1 print books and 1 single copy per eBook, with a limit of 5 eBooks per order."), "E-book added to cart message does not contain - You added");
                ...
                DriverContext.waitForPageToLoad();
                DriverContext.waitUntilElementIsVisible(productPageActions.txtSuccessMessage);
                Assertions.assertTrue(productPageActions.txtSuccessMessage.GetTextValue().contains("You added") || productPageActions.txtSuccessMessage.GetTextValue().contains("The maximum number of copies of a single product you may purchase is 1. Please contact a") || productPageActions.txtSuccessMessage.GetTextValue().contains("The maximum quantity you may purchase is limited to 1 print books and 1 single copy per eBook, with a limit of 5 eBooks per order."), "Journal added to cart message does not contain - You added");
    ...
    public void i_navigate_to_shipping_page_and_enter_the_shipping_details(DataTable table) {
        ...
        shippingPageActions.addShippingDetailsForEU(CucumberUtil.GetCellValueWithRowIndex("Street Address", 1),
    ...
    @And("I navigate to payment page and select different billing address and enter payment details in EU-HS")
    public void i_navigate_to_payment_page_and_select_different_billing_address_and_enter_payment_details(DataTable table) {
        List<Map<String, String>> billingAddress = table.asMaps(String.class, String.class);
        paymentPageActions.addDifferentBillingAddressEU(billingAddress);
        DriverContext.waitForPageToLoad();
        paymentPageActions.acceptTermsAndConditions();
